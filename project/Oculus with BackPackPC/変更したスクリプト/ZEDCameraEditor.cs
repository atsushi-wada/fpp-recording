...
public class ZEDCameraEditor : Editor
{
    ...
    private SerializedProperty svoOutputCompressionModeProperty; //元はこっち
  //public SerializedProperty svoOutputCompressionModeProperty;//できるか検証

   //Fukae
    private SerializedProperty unityRecord_or_notProperty;
    ...
    private void OnEnable()
    {
        ...
        //Fukae
        unityRecord_or_notProperty = serializedObject.FindProperty("unityRecord_or_not");
...
#pragma warning restore CS0809 // 旧形式のメンバーが、旧形式でないメンバーをオーバーライドします
    {
        ...
        //Debug.Log("cameraID is " + cameraIDProperty.enumValueIndex);
        ...
        if (showRecording.boolValue)
        {
            ...
            //Fukae
            GUIContent unityRecord_or_notLabel = new GUIContent("GameViewRecording", "If you use UnityRecorder, please check this.");
            unityRecord_or_notProperty.boolValue = EditorGUILayout.Toggle(unityRecord_or_notLabel, manager.unityRecord_or_not);
            //Fukae
            ...
            if (GUILayout.Button(recordLabel))
            {
                ...
                if (manager.needRecordFrame)
                {
                    ...
                    //File.AppendAllText(manager.csvOutputFileName, manager.timeList);
                ...
                else
                {
                    ...
                    if (manager.zedCamera.EnableRecording(manager.svoOutputFileName, (sl.SVO_COMPRESSION_MODE)svoOutputCompressionModeProperty.enumValueIndex) == sl.ERROR_CODE.SUCCESS)
                    {
